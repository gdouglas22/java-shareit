...
public class CurrentBookingStateStrategy implements BookingStateStrategy {
    ...
    public List<Booking> findByBooker(Long bookerId, Sort sort, LocalDateTime now) {
        return bookingRepository.findByBooker_IdAndStartIsBeforeAndEndIsAfter(bookerId, now, now, sort);
    }

    @Override
    public List<Booking> findByOwner(Long ownerId, Sort sort, LocalDateTime now) {
        return bookingRepository.findByItem_Owner_IdAndStartIsBeforeAndEndIsAfter(ownerId, now, now, sort);
    }
}
